...
**Summary:** Optional Distributed Tracing.
...
└── lib/
    └── elixir_scope/  # Or specific subdirectory like 'ast_repository'
                ElixirScope.Distributed.NodeCoordinator;elixir_scope_utils;elixir_scope_config;elixir_scope_events;elixir_scope_capture_core;elixir_scope_storage;distributed/
        └── lib/
            └── elixir_scope/
                └── distributed/
                    ├── global_clock.ex
...
*   [**EXPAND HERE: List 3-5 key public functions or GenServer calls. Example for `elixir_scope_ast_repo`:**]
    *   `ElixirScope.Distributed.Repository.get_cpg(module_name, function_name, arity)`
    *   `ElixirScope.Distributed.Repository.query_static_analysis(query_spec)`
    *   `ElixirScope.Distributed.CPGMath.calculate_centrality(cpg_data, node_id, type)`
    *   `ElixirScope.Distributed.CPGSemantics.detect_architectural_smells(cpg_data, opts)`
    *   `ElixirScope.Distributed.PatternMatcher.match_cpg_pattern(cpg_data, pattern_definition)`
...
Within the `TidewaveScope` ecosystem, the `elixir_scope_distributed` library will [**EXPAND HERE: Describe how `TidewaveScope`'s plug, MCP tools, or other ElixirScope libraries will interact with this one. Example for `elixir_scope_ast_repo`:**]
